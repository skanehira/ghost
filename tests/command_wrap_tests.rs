...
fn test_command_not_double_wrapped_on_restart() {
    ...
    let restart_result = commands::spawn(stored_command, task.cwd.clone().map(|c| c.into()), vec![]);
...
fn test_process_start_verification() {
    ...
    if !updated_tasks.is_empty() {
        ...
        assert_ne!(updated_tasks[0].status, storage::task_status::TaskStatus::Running);
    }
}
...
fn test_working_directory_preserved_on_restart() {
    ...
    let restart_result = commands::spawn(
        command,
